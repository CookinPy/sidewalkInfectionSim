...
################################################################################################################################################################
    def step(self):
        ...
        if self.sidewalk.isoccupied(sightx, desiredy) or self.sidewalk.isoccupied(sightx, desiredy-1) or self.sidewalk.isoccupied(sightx, desiredy+1):
            highy = desiredy + 1
            lowy = desiredy - 1
...
x = np.arange(len(results)) # sets up x var for plots
finalResults = pd.DataFrame.from_dict(results, orient='index', # converts results dictionary to pandas dataframe
