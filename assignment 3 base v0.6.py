    ...
    def step(self):
        ...
        if self.direction == 1: # Check which direction we're going
            ...
            if self.sidewalk.isoccupied(desiredx, desiredy):
                ...
                if direct > 0.5 and self.sidewalk.storage.check_coordinates(desiredx, highy):
                    if self.sidewalk.storage.isoccupied(desiredx, highy): #  and not self.sidewalk.storage.isoccupied(frontRight[0], frontRight[1])
                        desiredx = self.x
                        desiredy -= 1
                    elif self.sidewalk.storage.check_coordinates(desiredx, lowy):
                        if self.sidewalk.storage.isoccupied(desiredx, lowy): #  and not self.sidewalk.storage.isoccupied(frontRight[0], frontRight[1])
                            desiredx = self.x
                            desiredy += 1
                else:
                    if self.sidewalk.storage.check_coordinates(desiredx, lowy):
                        if self.sidewalk.storage.isoccupied(desiredx, lowy): #  and not self.sidewalk.storage.isoccupied(frontRight[0], frontRight[1])
                            desiredx = self.x
                            desiredy += 1
                        elif self.sidewalk.storage.check_coordinates(desiredx, highy):
                            if self.sidewalk.storage.isoccupied(desiredx, highy): #  and not self.sidewalk.storage.isoccupied(frontRight[0], frontRight[1])
                                desiredx = self.x
                                desiredy -= 1
        ...
        if self.direction == -1:
            ...
            if self.sidewalk.isoccupied(desiredx, desiredy):
                ...
                if direct > 0.5:
                    if self.sidewalk.storage.check_coordinates(desiredx, highy):
                        if self.sidewalk.storage.isoccupied(desiredx, highy): #  and not self.sidewalk.storage.isoccupied(frontRight[0], frontRight[1])
                            desiredx = self.x
                            desiredy -= 1
                        elif self.sidewalk.storage.check_coordinates(desiredx, lowy):
                    ...
                    if self.sidewalk.storage.check_coordinates(desiredx, lowy):
                        if self.sidewalk.storage.isoccupied(desiredx, lowy): #  and not self.sidewalk.storage.isoccupied(frontRight[0], frontRight[1])
                            ...
                            if self.sidewalk.storage.isoccupied(desiredx, highy): #  and not self.sidewalk.storage.isoccupied(frontRight[0], frontRight[1])
                                desiredx = self.x
                                desiredy -= 1
            # elif self.sidewalk.storage.isoccupied(frontRight[0], frontRight[1]): #  and not self.sidewalk.storage.isoccupied(frontLeft[0], frontLeft[1])
            #     desiredx = self.x
            #     desiredy += 1
...
##########################################################################
##########################################################################
    def run_step(self, time_step, id_count):   
        arrivalTimeLeft = random.expovariate(INTERARRIVAL)
        if arrivalTimeLeft <= time_step:
            id_count += 1
        ...
        arrivalTimeRight = random.expovariate(INTERARRIVAL)
        if arrivalTimeRight <= time_step:
            id_count += 1
            person = Person(id_count, self, -1)
            #self.storage.dic[id_count] = person
            person.enter_sidewalk(person.startx, person.starty)
            self.storage.add_item(person.x, person.y, person)
...
def updatefigure(*args):
    ...
    if t % 100 == 0:
        print("Time: %d" % t)
    id_count = sw.run_step(t, id_count)
    sw.refresh_image()
